...
func (c *Channel) InitChaincode(chaincodeId, funcName string, Args [][]byte) error {
	return c.invokeChaincode(chaincodeId, funcName, Args)
}

func (c *Channel) ExecChaincode(chaincodeId, funcName string, Args [][]byte) error {
	return c.invokeChaincode(chaincodeId, funcName, Args)
}

func (c *Channel) ReadChaincode(chaincodeId, funcName string, Args [][]byte) channel.Response {
	res, _ := c.client.Execute(
		channel.Request{ChaincodeID: chaincodeId, Fcn: funcName, Args: Args},
		channel.WithRetry(retry.DefaultChannelOpts),
	)
...
func (c *Channel) invokeChaincode(chaincodeId string, funcName string, Args [][]byte) error {
	if len(Args) == 0 {
		_, err := c.client.Execute(
			channel.Request{ChaincodeID: chaincodeId, Fcn: "InitLedger", IsInit: true},
			channel.WithRetry(retry.DefaultChannelOpts),
